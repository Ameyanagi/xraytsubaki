...
use xraytsubaki::prelude::{FeffFitResult, XASSpectrum};
...
fn settings_table(params: &PipelineParams) -> String {
    let v = serde_json::to_value(params).unwrap();
    let mut text = String::from("| Setting | Requested value |\n|---|---|\n");
...
impl Snapshot {
    pub(crate) fn context(&self) -> Value {
        json!({
            "software":{"name":"XrayTsubaki","version":env!("CARGO_PKG_VERSION")},
    ...
    pub(crate) fn markdown(&self) -> String {
        ...
        for fit in &self.project.fit_history {
            ...
            text.push_str("\n| Path | R_eff (Å) | Distance (Å) | ΔR (Å) | σ² (Å²) | S₀² | ΔE₀ (eV) |\n|---|---:|---:|---:|---:|---:|---:|\n");
            for p in &fit.path_details {
...
fn save_plot(plot: ruviz::prelude::Plot, path: &Path) -> Result<(), String> {
    plot.size_px(1600, 1000)
        .save(path)
        .map_err(|e| e.to_string())
}
...
pub(crate) fn export(snapshot: Snapshot, destination: &Path) -> Result<PathBuf, String> {
    ...
    crate::project::save(&destination.join("project.xtproj"), &snapshot.project)?;
    ...
    for (i, s) in snapshot.spectra.iter().enumerate() {
        ...
        match result {
            Ok(sp) => {
                ...
                for (name, plot) in spectrum_plots(
                    sp,
                    &s.path.file_name().unwrap_or_default().to_string_lossy(),
                ) {
                    let filename = format!("{id}-{name}.png");
                    match save_plot(plot,&figs.join(&filename)) {Ok(())=>assets.push(json!({"file":format!("figures/{filename}"),"source":s.path,"kind":name})),Err(e)=>errors.push(format!("{filename}: {e}"))}
                }
            }
    ...
    for (id, result) in &snapshot.results {
        ...
        for index in 0..result.datasets.len().max(1) {
            ...
            let plots = [
                ("k", plotting::build_fit_k(&view, &theme, true, None)),
                ("r", plotting::build_fit_r(&view, &theme, true, true, true)),
                ("q", plotting::build_fit_q(&view, &theme)),
                ("residual-k", plotting::build_fit_residual_k(&view, &theme)),
                ("residual-r", plotting::build_fit_residual_r(&view, &theme)),
            ];
            for (kind, plot) in plots {
                let filename = format!("fit-{id}-spectrum-{}-{kind}.png", index + 1);
                match save_plot(plot,&figs.join(&filename)) {Ok(())=>assets.push(json!({"file":format!("figures/{filename}"),"fit_id":id,"dataset_index":index,"kind":kind})),Err(e)=>errors.push(format!("{filename}: {e}"))}
            }
        }
    }
    ...
    write_json(
        ...
        &json!({"figures":assets,"notices":errors,"batch_results_stale":snapshot.batch_stale,"project_paths":"Original source paths are retained; source spectra and FEFF files are not bundled."}),
    )?;
    let mut index = String::from(
        "# Publication assets\n\n[Analysis and history](analysis.md) · [Resolved processing](resolved.md) · [Methods draft](methods.md) · [References](references.md) · [State JSON](state.json) · [Project](project.xtproj)\n\nThis folder is a local export. Review the methods and reference choices before publication. The project retains original source paths; processed arrays are included in data/.\n\n## Figures\n\n",
    );
    for asset in &assets {
...
mod tests {
    ...
    fn copper_export_has_resolved_settings_arrays_and_readable_pngs() {
        ...
        for asset in manifest["figures"].as_array().unwrap() {
            ...
            assert_eq!(u32::from_be_bytes(bytes[20..24].try_into().unwrap()), 1000);
        }
        if std::env::var_os("XTS_KEEP_PUBLICATION_QA").is_some() {
