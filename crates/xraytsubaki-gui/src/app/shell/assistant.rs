            ...
            let opened = cx.open_window(
                gpui::WindowOptions {
                    ...
                    titlebar: Some(gpui::TitlebarOptions {
                        title: Some("XrayTsubaki Assistant".into()),
                        ..Default::default()
                    }),
                    ..Default::default()
                },
...
impl AssistantWindow {
    ...
    fn receive(&mut self, v: Value, cx: &mut Context<Self>) {
        if std::env::var_os("XTS_ASSISTANT_TRACE").is_some() {
                ...
                _ => {
                    if let Some(id) = v.get("id") {
                        if let Some(c) = &self.client {
                            let _=c.send(json!({"id":id,"error":{"code":-32601,"message":"This client supports XrayTsubaki analysis tools only. Ask the user in your reply instead."}}));
                        }
                    }
                }
            }
