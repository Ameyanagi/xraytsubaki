...
use xraytsubaki::xafs::fitting::FitSolverReport;
...
pub(crate) fn source_template(
    template: xraytsubaki::xafs::fitting::template::ParameterTemplate,
    selected: &[xraytsubaki::xafs::structure::PathInfo],
    paths: &[FitPathSpec],
) -> xraytsubaki::xafs::fitting::template::TemplateResult {
    use xraytsubaki::xafs::fitting::template::{TemplateResult, apply_template};
...
mod tests {
    ...
    fn energy_shift_name_resolves_as_variable() {
        let value = xraytsubaki::xafs::fitting::variables::eval_expression_with("e0", |name| {
            assert_eq!(name, "e0");
            Ok(7.25)
        })
        .unwrap();
    ...
    fn default_template_recovers_known_energy_shift() {
        ...
        use xraytsubaki::xafs::structure::rank_paths;
        let file = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../xraytsubaki/tests/testfiles/feffcu01.dat");
    ...
    fn structures_have_independent_shell_parameters() {
        use xraytsubaki::xafs::fitting::template::ParameterTemplate;
        use xraytsubaki::xafs::structure::PathInfo;
        let paths = vec![
    ...
    #[test]
    #[ignore = "requires XTS_COMPARE_PROJECT with local paths and a standalone spectrum"]
    fn diagnose_saved_fit() {
        let file = std::env::var("XTS_COMPARE_PROJECT").expect("XTS_COMPARE_PROJECT");
    ...
    #[test]
    #[ignore = "requires XTS_COMPARE_PROJECT and both calculation backends"]
    ...
    fn metal_foils_fit_with_both_backends() {
        let _guard = crate::feffgen::feff_test_lock();
        let core = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../xraytsubaki");
            ...
            [("Cu", "cu", "cu_150k.xmu"), ("Ni", "ni", "ni_metal_rt.xdi")]
        {
            ...
            for (backend, mode) in [
                ...
                crate::project::save(&ws.join("foil.xtproj"), &project).unwrap();
