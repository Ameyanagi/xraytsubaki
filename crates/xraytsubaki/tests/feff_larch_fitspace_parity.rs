...
use xraytsubaki::xafs::fitting::estimate_noise;
...
fn load_metadata() -> FitspaceMetadata {
    ...
    assert!(
        meta_path.exists(),
        "missing Larch fit-space metadata at {}. regenerate with: uv run --project crates/xraytsubaki/tests/pythonscript python crates/xraytsubaki/scripts/generate_larch_fitspace_references.py",
        meta_path.display()
    );
