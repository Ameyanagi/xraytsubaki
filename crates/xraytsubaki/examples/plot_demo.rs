...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use xraytsubaki::xafs::{FittingError, XAFSError};
    ...
    fn discover_feff85l_rdinp() -> Option<PathBuf> {
        let env_candidates = ["XRAYTSUBAKI_FEFF8L_RDINP", "FEFF8L_RDINP", "FEFF85L_RDINP"];
    ...
    if !refs_root.exists() {
        return Err(format!(
            "missing larch references at {}\nrun: uv run --with xraylarch python crates/xraytsubaki/scripts/generate_larch_fit_references.py",
            refs_root.display()
        )
        .into());
    }
    ...
    let feff8l_rdinp = discover_feff85l_rdinp().ok_or_else(|| {
        "failed to locate feff8l_rdinp executable. set XRAYTSUBAKI_FEFF8L_RDINP or install xraylarch FEFF binaries".to_string()
    })?;
