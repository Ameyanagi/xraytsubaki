...
use rexafs::xafs::fitting::{parse_feff_path_file, FeffFlavor};
...
fn fixture(name: &str) -> (rexafs::xafs::structure::Structure, Value) {
...
fn assert_sites_match(s: &rexafs::xafs::structure::Structure, json: &Value, tol: f64) {
...
fn structure_feff_inp_runs_through_refeff() {
    use rexafs::prelude::{run_feff_and_load_paths, FeffExecutionMode, FeffRunRequest};
    let (s, _) = fixture("ru_hcp");
    let cluster = build_cluster(
        &s,
        &AbsorberSelection::Element("Ru".into()),
...
fn structure_amcsd_subset_search_and_fetch() {
    use rexafs::xafs::structure::db::amcsd::Amcsd;
...
fn structure_materials_project_fixture_converts() {
    use rexafs::xafs::structure::db::mp::{
...
fn structure_materials_project_live() {
    use rexafs::xafs::structure::db::mp::{MaterialsProject, MaterialsProjectConfig};
