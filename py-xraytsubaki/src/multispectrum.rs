...
impl PyConstrainedParameters {
    ...
    fn constrain(&mut self, name: &str) -> PyResult<PyConstraintBuilder> {
...
#[pyclass]
pub struct PyConstraintBuilder<'a> {
    parameters: &'a mut PyConstrainedParameters,
    target_name: String,
}

#[pymethods]
impl<'a> PyConstraintBuilder<'a> {
    /// Set a direct reference constraint
    fn reference_from(&mut self, reference: &str) -> PyResult<&mut PyConstrainedParameters> {
        if let Some(param) = self.parameters.parameters.get_mut(&self.target_name) {
    ...
    fn scale_from(&mut self, reference: &str, factor_param: &str) -> PyResult<&mut PyConstrainedParameters> {
        if let Some(param) = self.parameters.parameters.get_mut(&self.target_name) {
            if let Some(factor_param) = self.parameters.parameters.get(factor_param) {
                let factor = factor_param.param.value;
                ...
                Err(PyValueError::new_err(format!("No parameter named '{}'", factor_param)))
            }
        } else {
            Err(PyValueError::new_err(format!("No parameter named '{}'", self.target_name)))
        }
    ...
    fn offset_from(&mut self, reference: &str, offset_param: &str) -> PyResult<&mut PyConstrainedParameters> {
        if let Some(param) = self.parameters.parameters.get_mut(&self.target_name) {
            if let Some(offset_param) = self.parameters.parameters.get(offset_param) {
                let offset = offset_param.param.value;
                ...
                Err(PyValueError::new_err(format!("No parameter named '{}'", offset_param)))
            }
        } else {
            Err(PyValueError::new_err(format!("No parameter named '{}'", self.target_name)))
        }
