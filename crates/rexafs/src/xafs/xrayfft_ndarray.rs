...
impl XrayFFTF {
    ...
    pub fn xftf(
        ...
        // The length of r is different by 1 between xraylarch and rexafs. This is due to the implementation of FFT.
        let irmax =
...
pub fn xftf_fast(chi: ArrayBase<ViewRepr<&f64>, Ix1>, nfft: usize, kstep: f64) -> DynRealDft<f64> {
    ...
    for (output, input) in cchi.iter_mut().zip(chi.iter()) {
        *output = *input;
    }
...
pub fn xftf_fast_nalgebra(chi: &DVector<f64>, nfft: usize, kstep: f64) -> DynRealDft<f64> {
    let mut cchi = vec![0.0_f64; nfft];
    let length = chi.len().min(nfft);
    cchi[..length].copy_from_slice(&chi.as_slice()[..length]);
