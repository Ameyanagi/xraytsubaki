...
impl AUTOBK {
    ...
    fn solve_trust_region_problem(problem: AUTOBKSpline) -> Result<AUTOBKSpline, BackgroundError> {
        ...
        let coefs_key = apex_problem.add_variable(
            ManifoldType::RN,
            ApexDVector::from_column_slice(problem.coefs.as_slice()),
        );
        apex_problem.add_residual_block(&[coefs_key], Box::new(factor), None);
        ...
        let result = optimizer.optimize(&mut apex_problem).map_err(|err| {
...
fn spline_eval_nalgebra(
    ...
    // experimental
    let bkg_out = DVector::from_vec(spline::evaluate(
        &knots_vec,
        &coefs_vec,
        order,
        kout.as_slice(),
...
impl AUTOBKSpline {
    ...
    pub fn residual_jacobian_with_scale(
        ...
        // the B-spline basis over the knots, order and evaluation points. Those three
...
impl ApexFactor for AUTOBKSplineFactor {
    fn linearize(
        ...
        let coefs = params
            ...
            .map(|param| DVector::from_column_slice(param))
            .unwrap_or_else(|| self.spline.coefs.clone());
        let base = self.spline.residual_vec(&coefs);
        residual.copy_from_slice(base.as_slice());
        if let Some(mut output) = jacobian {
            let values = self.spline.residual_jacobian(&coefs);
    ...
    fn jacobian_shape(&self) -> (usize, usize) {
        (self.residual_len, self.spline.coefs.len())
    }
}
