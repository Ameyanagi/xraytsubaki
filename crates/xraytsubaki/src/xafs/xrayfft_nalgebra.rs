...
impl XrayFFTF {
    ...
    pub fn get_chir_real(&self) -> Option<DVector<f64>> {
        ...
        Some(DVector::from_iterator(len_r, chir.iter().take(len_r).map(|x| x.re)))
    }

    pub fn get_chir_imag(&self) -> Option<DVector<f64>> {
        let len_r = self.r.as_ref()?.len();
        let chir = self.chir.as_ref()?;
        Some(DVector::from_iterator(len_r, chir.iter().take(len_r).map(|x| x.im)))
    }
...
impl XrayFFTR {
    ...
    pub fn xftr(&mut self, r: &DVector<f64>, chir: &DynRealDft<f64>) -> Result<&mut Self, FFTError> {
        self.fill_parameter(r);

        let win = ftwindow(r, self.rmin, self.rmax, self.dr, self.dr2, self.window).map_err(|e| {
