...
use xraytsubaki::xafs::structure::{self as core, StructureSource};
...
pub fn load_path_geometry(file: &Path, cluster: Option<&core::Cluster>) -> Option<PathGeometry> {
    let core_geo = xraytsubaki::prelude::parse_feff_path_file(
...
mod tests {
    ...
    fn cif_import_and_local_library_provider() {
        let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../xraytsubaki/tests/testfiles/cif");
