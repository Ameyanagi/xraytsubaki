    ...
    fn pick_molecule_atom(&mut self, position: Point<Pixels>, cx: &mut Context<Self>) {
        ...
        let mut hits: Vec<_> = scene
            ...
            .filter_map(|(index, a)| {
                ...
                (d <= 14.).then_some((
                    i,
                    if a.absorber && self.structure.highlight_absorber {
                        f32::INFINITY
                    } else {
                        p[2]
                    },
...
fn paint_scene(
    ...
    for (i, a) in scene.atoms.iter().enumerate() {
        if atom_in_style(scene, style, i) && depth.atom_alpha(a.pos) > 0.002 {
            // Highlight is an opaque color overlay so foreground atoms cannot hide it.
            let z = if a.absorber && highlight_absorber {
                f32::INFINITY
            } else {
                project(a.pos)[2]
            };
            draw.push((z, Primitive::Atom(i)));
        }
    }
    ...
    for (_, p) in draw {
        match p {
            ...
            Primitive::Atom(i) => {
                ...
                color.a = if a.absorber && highlight_absorber {
                    1.
                } else {
                    (if outside { 0.14 } else { 1. }) * depth.atom_alpha(a.pos)
                };
    ...
    // Annotation is drawn last so the absorber remains identifiable behind
    // foreground bonds/atoms. The highlighted sphere is a color overlay too.
    ...
    fn rutile_repeats_complete_titanium_oxygen_octahedra() {
        let s = core::read_cif(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../xraytsubaki/data/builtin_cifs/tio2_rutile.cif"),
        )
        .unwrap();
