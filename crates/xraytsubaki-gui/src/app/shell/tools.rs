...
use xraytsubaki::prelude::{AnalysisSpace, LcfConfig, PcaConfig};
...
pub struct AnalysisState {
    pub lcf: Option<xraytsubaki::prelude::LcfResult>,
    /// Ranked combinations ("fit all combinations"), best first.
    pub ranked: Vec<xraytsubaki::prelude::LcfResult>,
    pub pca: Option<xraytsubaki::prelude::PcaModel>,
    pub pca_fit: Option<xraytsubaki::prelude::PcaFit>,
        ...
        let outcome: Result<String, String> = match tool {
            Tool::Lcf => {
                if spectra.len() < 2 {
                    ...
                    xraytsubaki::prelude::lcf_combinatorial(&unknown, &spectra, &cfg, 4)
                    ...
                    xraytsubaki::prelude::lcf(&unknown, &spectra, &cfg)
                        .map_err(|e| e.to_string())
                        .map(|mut r| {
                            relabel(&mut r, &names);
            ...
            Tool::Pca => {
                if spectra.len() < 2 {
                    ...
                    xraytsubaki::prelude::pca_train(&spectra, &pcfg)
                        .map_err(|e| e.to_string())
                        .and_then(|model| {
...
fn relabel(result: &mut xraytsubaki::prelude::LcfResult, names: &[String]) {
